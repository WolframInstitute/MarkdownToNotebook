...
The eight single-qubit gates are stored as an [Association]() of $2 \times 2$ unitary matrices; `blochVector` projects a pure state $|\psi\rangle = a|0\rangle + b|1\rangle$ onto the unit sphere by computing the Bloch coordinates $(2\Re(a^* b),\ 2\Im(a^* b),\ |a|^2 - |b|^2)$; `trajectory` folds the gates over the initial state and returns every intermediate vector.
