...
- The persona is a *Chat Notebook* style assistant: address it as `` `AdaLovelace `` in a Chat Notebook, or pass it as `LLMPrompt["AdaLovelace"]` to [LLMSynthesize]() / [LLMResourceFunction]().
...
- Pair with [`LLMConfiguration`]() to override the model or temperature - a slightly higher temperature (~0.8) brings out more of her poetical metaphor.
