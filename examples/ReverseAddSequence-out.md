...
The function adds an integer to its digit-reversal and repeats until the value becomes a palindrome (or a step cap is reached). The whole trajectory is returned, so the caller sees every intermediate value.
...
<code>[ReverseAddSequence]()[$n$]</code> gives the trajectory of the reverse-and-add iteration starting from the integer $n$, stopping at the first palindromic value.

<code>[ReverseAddSequence]()[$n$, $max$]</code> stops after at most $max$ iterations even if no palindrome was reached.
...
![output](images/ReverseAddSequence-out-1.png)
...
![output](images/ReverseAddSequence-out-2.png)
...
![output](images/ReverseAddSequence-out-3.png)
...
![output](images/ReverseAddSequence-out-4.png)
...
![output](images/ReverseAddSequence-out-5.png)
...
![output](images/ReverseAddSequence-out-6.png)
